 ...
 */
	...
	try {
		...
		$paymentdata->paymentid= $item->id;
		$paymentdata->paymentamount= $item->amount;
		...
		if ($paymentdata->eventType=== "Payment") {
			echo " paymentdata : " . json_encode($paymentdata). "<br>" ;
            ...
            fwrite($f," *** Error ipnreturn :  " . date("d-m-y h:i:s") .  $e->getMessage() . "\n");
            fclose($f);
        }
