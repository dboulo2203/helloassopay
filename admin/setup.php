...
/**
 * \file    helloassopay/admin/setup.php
 * \ingroup helloassopay
 * \brief   HelloAssoPay setup page.
 */
...
$hookmanager->initHooks(array('helloassopaysetup', 'globalsetup'));
...
// Setup conf HELLOASSOPAY_MYPARAM1 as a simple string input
$item = $formSetup->newItem('HELLOASSOPAY_CLIENT_SECRET');
$item->defaultFieldValue = 'default value';

// Setup conf HELLOASSOPAY_MYPARAM1 as a simple string input
$item = $formSetup->newItem('HELLOASSOPAY_ORGANISM_SLUR');
$item->defaultFieldValue = 'default value';

// Setup conf HELLOASSOPAY_MYPARAM1 as a simple string input
$item = $formSetup->newItem('HELLOASSO_PAYMENTMODE');
$item->defaultFieldValue = 'default value';

// Setup conf HELLOASSOPAY_MYPARAM1 as a simple string input
$item = $formSetup->newItem('HELLOASSO_BANK_ACCOUNT_FOR_PAYMENTS');
$item->defaultFieldValue = 'default value';
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir . "core/modules/helloassopay/doc/pdf_" . $modele . "_" . strtolower($tmpobjectkey) . ".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if ($filefound) {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: " . DOL_URL_ROOT . "/document.php?modulepart=helloassopay-" . strtolower($tmpobjectkey) . "&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'HELLOASSOPAY_' . strtoupper($tmpobjectkey) . "_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'HELLOASSOPAY_' . strtoupper($tmpobjectkey) . '_ADDON_PDF';
...
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, "helloassopay@helloassopay");

// Setup page goes here
echo '<span class="opacitymedium">' . $langs->trans("HelloAssoPaySetupPage") . '</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_' . strtolower($myTmpObjectKey) . '_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'HELLOASSOPAY_' . strtoupper($myTmpObjectKey) . '_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir . '/' . $file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'HELLOASSOPAY_' . strtoupper($myTmpObjectKey) . '_ADDON_PDF';
