<?php
/**
 * Copyright © Lyra Network.
 * This file is part of Lyra Collect plugin for Dolibarr. See COPYING.md for license details.
 
 */

/**
 * This file contains all methods to be executed as hooks.
 */

$langs->load("up2pay@up2pay");

/**
 * Class Actionsup2pay
 */
class ActionsHelloassopay_deprecated
{
    /**
     * @var DoliDB database handler.
     */
    public $db;

    /**
     * @var string error code (or message).
     */
    public $error = '';

    /**
     * @var array errors.
     */
    public $errors = array();

    /**
     * @var array Hook results. Propagated to $hookmanager->resArray for later reuse.
     */
    public $results = array();

    /**
     * @var string displayed by executeHook() immediately after return.
     */
    public $resprints;

    /**
     * Constructor.
     *
     *  @param    DoliDB        $db      Database handler
     */
    public function __construct($db)
    {
        $this->db = $db;
    }

    /**
     * Execute action.
     *
     * @param    array           $parameters     Array of parameters
     * @param    CommonObject    $object         The object to process (an invoice if you are in invoice module, a propale in propale's module, etc...)
     * @param    string          $action         'add', 'update', 'view'
     * @return   int                             <0 if KO,
     *                                           =0 if OK but we want to process standard actions too,
     *                                           >0 if OK and we want to replace standard actions
     */
    public function getNomUrl($parameters, &$object, &$action)
    {
        $this->resprints = '';
        return 0;
    }

   
   /**
     * Print Lyra Payment Button on screen.
     *
     * @param   array            $parameters     Array of parameters
     * @param   Object            $object         Object output
     * @param   string            $action         'add', 'update', 'view'
     * @param   HookManager       $hookmanager    Hook manager propagated to allow calling another hook
     * @return  int                               < 0 on error, 0 on success, 1 to replace standard code
     */
    public function doAddButton($parameters, &$object, &$action, $hookmanager)
    {
       dol_include_once('/custom/helloassopay/lib/helloassopay.lib.php');

        global $conf, $langs;

        $error = 0; // Error counter.
        // $source = GETPOST("s", 'alpha') ? GETPOST("s", 'alpha') : GETPOST("source", 'alpha');

        if (in_array($parameters['currentcontext'], array('newpayment'))) { // Do something only for the context 'somecontext1' or 'somecontext2'.
           $paymentmethod = $parameters['paymentmethod'];

             if (! empty($conf->helloassopay->enabled) && (empty($paymentmethod) || paymentmethod=='helloassopay')) {
            //     dol_htmloutput_mesg($langs->trans('LYRA_UNSUPPORTED_CURRENCY_MESSAGE'), '', 'warning');
            // } elseif ((empty($paymentmethod) || $paymentmethod == 'lyra') && ! empty($conf->lyra->enabled) && lyraCanUseCurrency()) {
            //     if ($conf->global->LYRA_CTX_MODE == 'TEST' || GETPOST('forcesandbox', 'int')) { // We can force sandbox with param 'forcesandbox'.
            //         dol_htmloutput_mesg($langs->trans('LYRA_SANDBOX_MESSAGE'), '', 'warning');
            //     }

            //     if (empty($conf->banque->enabled) || empty($conf->global->LYRA_DOLIBARR_BANK_ACCOUNT) || ($conf->global->LYRA_DOLIBARR_BANK_ACCOUNT == -1)) {
            //         dol_htmloutput_mesg($langs->trans('LYRA_WARNING_BANK'), '', 'warning');
            //     }
                    $result =  '<br>';
                    $result .= '<div class="button buttonpayment" id="div_dopayment_helloassopay">';
                    $result .= '<span class="fa fa-credit-card"></span>';
                    $result .= '<input class="" type="submit" id="dopayment_helloassopay" name="dopayment_helloassopay value="' . $langs->trans("Payer") . '">';
                    $result .= '<br>';
                    $result .= '<span class="buttonpaymentsmall">' . 'helloassopay button' . '</span>';
                    $result .= '</div>';
                    $result .= '<script>
                                    $( document ).ready(function() {
                                        $("#div_dopayment_helloassopay").click(function() {
                                            $("#dopayment_helloassopay").click();
                                        });
                                        $("#dopayment_helloassopay").click(function(e) {
                                            $("#div_dopayment_helloassopay").css( \'cursor\', \'wait\' );
                                            e.stopPropagation();
                                            return true;
                                        });
                                    });
                                </script>
                                ';
                    print $result;
                    $this->resprints = $result;
               

                return $this->resprints;
            }
        }
        

        if (! $error) {
            return 0; // Or return 1 to replace standard code.
        }

        $this->errors[] = 'Error message';
        return -1;
    }
    /**
     * Set Lyra as a valid payment.
     *
     * @param   array            $parameters   Array of parameters
     * @param   Object           $object       Object output
     * @param   string           $action       'add', 'update', 'view'
     * @param   HookManager      $hookmanager  Hook manager propagated to allow calling another hook
     * @return  int                            < 0 on error, 0 on success, 1 to replace standard code
     */
    public function doValidatePayment($parameters, &$object, &$action, $hookmanager)
    {
        dol_include_once('/helloassopay/lib/helloassopay.lib.php');

        global $conf, $langs;

        if (in_array($parameters['currentcontext'], array('newpayment'))) { // Do something only for the context 'somecontext1' or 'somecontext2'.
            $paymentmethod = $parameters['paymentmethod'];
		 if ((empty($paymentmethod) || $paymentmethod == 'helloassopay') && ! empty($conf->up2pay->enabled)) {
        //         if (lyraCanUseCurrency() || $conf->global->LYRA_CTX_MODE == 'TEST' || GETPOST('forcesandbox', 'int')) {
			$langs->load("helloassopay");
            $parameters['validpaymentmethod']['helloassopay'] = 'valid';
		}
        //     }
        // }
		}
        return 0;
    }

       /**
     * Check status of $object (invoice, order, donation...) to show a message in newpayment.php.
     *
     * @param   array             $parameters   Array of parameters
     * @param   Object            $object       Object output
     * @param   string            $action       'add', 'update', 'view'
     * @param   HookManager       $hookmanager  Hook manager propagated to allow calling another hook
     * @return  int                             < 0 on error, 0 on success, 1 to replace standard code
     */
    public function doCheckStatus($parameters, &$object, &$action, $hookmanager)
    {
        global $conf, $langs;

        $error = 0; // Error counter.
        if (in_array($parameters['currentcontext'], array('newpayment'))) { // Do something only for the context 'somecontext1' or 'somecontext2'.
             $paymentmethod = $parameters['paymentmethod'];
            $source = $parameters['source'];
            $object = $parameters['object'];

             if ((empty($paymentmethod) || $paymentmethod == 'lyra') && ! empty($conf->lyra->enabled)) {
                 if ($source == 'order' && $object->billed) {
                     print '<br><br><span class="amountpaymentcomplete">' . $langs->trans("OrderBilledPending") . '</span>';
                 }

                 if ($source == 'invoice' && strripos($object->note_private, '##')) { // The last appearance is found.
                     print '<br><br><span class="amountpaymentcomplete">' . $langs->trans("LYRA_PENDING_PAYMENT_DESC") . '</span>';
                     exit;
                 }

                 if ($source == 'donation' && $object->paid) {
                     print '<br><br><span class="amountpaymentcomplete">' . $langs->trans("DonationPaidPending") . '</span>';
                 }
             }
         }
           $source = $parameters['source'];
            $object = $parameters['object'];

        if (! $error) {
            return 0; // Or return 1 to replace standard code.
         }

        $this->errors[] = 'Error messageA';
        return -1;
    }

    /**
     * Start payment process with up2pay.
     *
     * @param   array        $parameters    Array of parameters
     * @param   Object       $object        Object output
     * @param   string       $action        'add', 'update', 'view'
     * @param   HookManager  $hookmanager   Hook manager propagated to allow calling another hook
     * @return  int                        < 0 on error, 0 on success, 1 to replace standard code
     */
    public function doPayment($parameters, &$object, &$action, $hookmanager)
    {
       global  $db;
       global $conf;
       dol_include_once('/helloassopay/lib/helloassopay.lib.php');
       dol_include_once('/helloassopay/core/modulesmodhelloassopay.class.php');

       // require '../../../main.inc.php';
        include_once DOL_DOCUMENT_ROOT.'/compta/facture/class/facture.class.php';

        require_once DOL_DOCUMENT_ROOT.'/core/lib/company.lib.php';
        require_once DOL_DOCUMENT_ROOT.'/core/lib/payments.lib.php';

        // Get the invoice
        include_once DOL_DOCUMENT_ROOT.'/compta/facture/class/facture.class.php';

        // log_in_file("----------- DoPayment " . json_encode($parameters). " -----------" . "\n");

        try {
            if (in_array($parameters['currentcontext'], array('newpayment'))) {
                if ($parameters['paymentmethod'] == 'helloassopay') {
                    $tag = (empty($parameters['tag']) ? GETPOST("ref", 'alpha') : $parameters['tag']);

                    // log_in_file("----------- DoPayment redirect " . $tag. " -----------" . "\n");
                    
                    $facture = new Facture($db);
                    $result = $facture->fetch(null,$tag);		       
                    
                    if (!$result) {
                        throw new Exception('Invoice not found : ' . $tag); 
                    }
                    $invoice_id= $facture->id;
                    // Get customer
                    $thirdparty_id=$facture->socid;
                    $thirdparty = new Societe($db);
                    $resultThirdparty= $thirdparty->fetch($thirdparty_id);
                    if (!$resultThirdparty){
                        throw new Exception('thirdparty not found :' . $thirdparty_id); 		
                    }

                    // Send form to Up2Pay
                    // log_in_file(" ***  DoPayment : " . $facture->total_ttc ." -" . "EUR" ." -" . $thirdparty->email ." -" . $urlok  ." -" . $urlko  ." -" . $tag);
                    print_helloassopay_redirect($facture->total_ttc, "EUR", $thirdparty->email, $urlok, $urlko, $tag);
                
                    return 0;
            }
            }
        } catch(Exception $exp) {
            log_in_file(" 	*** Error : ".	$exp->getMessage() . "\n");
            return -1;
        }		


        $this->errors[] = 'Error message';
        return -1;
    }
 
}
