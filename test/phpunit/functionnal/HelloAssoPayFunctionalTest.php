...
/**
 * \file    test/phpunit/HelloAssoPayFunctionalTest.php
...
/**
 * Class HelloAssoPayFunctionalTest
 *
 ...
 *
 * @package Testhelloassopay
 */
class HelloAssoPayFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modHelloAssoPay')->click();
		} else {
			// Disable the module
			$this->byHref('modHelloAssoPay')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/helloassopay/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('helloassopay/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/helloassopay/admin/about.php');
		$this->authenticate();
		return $this->assertContains('helloassopay/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('helloassopaywidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modHelloAssoPay_HelloAssoPayTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modHelloAssoPay_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
